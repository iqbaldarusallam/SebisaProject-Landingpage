...
const CLOUDINARY_FOLDER = "sebisa/portfolios";
...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File)) {
      return Response.json(errorResponse("File portofolio wajib diupload"), {
        status: 400,
      });
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
      return Response.json(errorResponse("Ukuran gambar maksimal 4MB"), {
        status: 400,
      });
    }
      ...
      errorResponse(
        error instanceof Error && error.message.includes("timeout")
          ...
          : error instanceof Error
            ? error.message
            : "Gagal upload gambar portofolio",
      ),
      { status: 500 },
    );
  }
